...
def _mde_confirma_top_safe(bot_top: str | None) -> bool:
    try:
        return bool(_mde_confirma_top(bot_top))
    except Exception as e:
        try:
            agregar_evento(f"⚠️ MDE confirm fallback: {e}")
        except Exception:
            pass
        return False
                ...
                for bot in BOT_NAMES:
                    ...
                    if not activo_real:
                        ...
                        try:
                            ...
                            mde_activo = False
                            agregar_evento(f"⚠️ MDE desactivado por fallback: {e}")
                        ...
                        if MODO_REAL_MANUAL:
                            ...
                            if candidatos and (PENDIENTE_FORZAR_BOT is None) and (owner in (None, "none")):
                                ...
                                if mejor is not None:
                                    ...
                                    if mde_activo and (not _mde_confirma_top_safe(mejor_bot)):
